...
const setup = async () => {
    ...
    l2ceg = l2CEGFactory.attach(L1CEGAddr)
    ...
    // set gateway
    let res = await l1gr.setERC20Gateway([l1token.address], [L1CEGAddr]).connect(walletL1)
    let rec = await res.wait()
    console.log(`L1 router set ERC20 gateway ${rec.status == 1}`)
    res = await l2gr.setERC20Gateway([l2token.address], [L2CEGAddr]).connect(walletL2)
    rec = await res.wait()
    console.log(`L2 router set ERC20 gateway ${rec.status == 1}`)
    // update token mapping
    res = await l1ceg.updateTokenMapping(l1token.address, l2token.address).connect(walletL1)
    rec = await res.wait()
    console.log(`L1 custom gateway update token mapping ${rec.status == 1}`)
    res = await l2ceg.updateTokenMapping(l2token.address, l1token.address).connect(walletL2)
    rec = await res.wait()
